...
def main(sample_file, to_code):
    ...
    move_mirror(old_record, new_record)
...
def recode_record(old_record, to_code):
    ...
    new_record['media_urls'] = recode_mirrors(old_record['media_urls'],
                                              old_record['language'], to_code)

    return new_record


def recode_mirrors(mirrors, from_code, to_code):
    return [m for m in mirrors
            if 'mirror.widelanguageindex.org' not in m]
...
def move_mirror(old_record, new_record):
    old_suffix = '{language}/{language}-{checksum}.mp3'.format(**old_record)

    mirror.unmirror(old_suffix)
